...
# ... 加载您的模型 ...
model_path = "/app/model/model_randomforest.joblib" # 将 'your_model_path.pkl' 替换为模型的实际路径
...
class Signal_open(bt.Indicator):
    ...
    def next(self):
        if len(self.data) >= self.p.data_length + 1:  # 需要额外 60 天数据。len(self.data) 代表喂到next里的数据长度
            ...
            # 组合数据
            data_inputs = np.array(today_ohlcv + past_close + past_volume).reshape(1, -1)

            # 调用您的深度学习模型进行预测
            try:

                signal_indic = model.predict(data_inputs)[0] # 返回值是数组，虽然只有一个值，但也要用[0]取出来
                if signal_indic not in [1, -1, 0]:
                    signal_indic = 0
...
class ml_randomforest(bt.Strategy):
    params = (
        ('stop_loss_pct', 0.005),
        ('stop_earn_pct', 0.05),
        ('data_name', ''),
